...
**Approach 1: Permanent boundary (Kutrib 2009).** The boundary `#` is not a cell state — it's a fixed symbol that appears as neighbor input for border cells but can never be produced by δ. The transition function has domain $(S \cup \{\#\})^3 \to S$.
...
|---|---|---|---|
| Tuple | $\langle S, \delta, \#, A, F \rangle$ | `CellAutomaton α β Q` with `δ`, `embed`, `project` | Ours is more general (transducer view) |
